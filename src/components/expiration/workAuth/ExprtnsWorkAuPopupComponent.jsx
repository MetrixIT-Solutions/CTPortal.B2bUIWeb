...
const ExprtnsWorkAuPopupComponent = (props) => {
  const {modal, expObj, status, num, strDt, expDt, receiptNum, position, uscisNum, sevisNum, errMsg, disabled, isVrfdModal, exprActList} = props.state;
  const {setStateData, handleWorkAthUpdate, handleVrfyUpdate} = props;
  ...
  return (
    <>
      <Modal show={modal} onHide={() => setStateData({modal: false, recordId: '', oldObj: {}, status: '', num: '', strDt: '', expDt: '', receiptNum: '', position: '', uscisNum: '', sevisNum: '', errMsg: ''})} size='lg'>
              ...
              {status == 'CPT' && (
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <span className='text-danger'>*</span>
                    <select className='form-select form-control' id='position' name='position' value={position} onChange={(e) => setStateData({position: e.target.value, errMsg: ''})}>
                ...
                <>
                  {status !== 'H4' && status !== 'GCEAD' && status !== 'GC' && status !== 'L2' && status !== 'H4EAD' && status !== 'Citizen' && status !== 'Canadian' && status !== 'Other' ? (
                    <div className='col-md-6'>
                      <div className='form-group'>
                        ...
                        <span className='text-danger'>*</span>
                        <input type='text' className='form-control' placeholder='Enter Card Number' name='cardNum' value={num} onChange={(e) => setStateData({num: e.target.value, errMsg: ''})} />
                      </div>
                    </div>
                  ) : (
                    ''
                  ...
                  <div className='col-md-6'>
                    <div className='form-group'>
                      ...
                      <span className='text-danger'>*</span>
                      <input type='text' className='form-control' placeholder='Enter USCIS Number' maxLength={20} name='USCISNum' value={uscisNum} onChange={(e) => setStateData({uscisNum: e.target.value, errMsg: ''})} />
                    </div>
                  </div>
                </>
              )}
              {(status == 'CPT' || status == 'OPT' || status == 'Stem OPT') && (
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <span className='text-danger'>*</span>
                    <input type='text' className='form-control' placeholder='Sevis Number' name='sevisNum' value={sevisNum} onChange={(e) => setStateData({sevisNum: e.target.value, errMsg: ''})} />
                  </div>
                </div>
              )}
              {status == 'H1B' && (
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <span className='text-danger'>*</span>
                    <input type='text' className='form-control' placeholder='Enter Receipt Number' name='receiptNum' value={receiptNum} onChange={(e) => setStateData({receiptNum: e.target.value, errMsg: ''})} />
                  </div>
                </div>
              )}
              <div className='col-md-6'>
              ...
              <div className='col-md-12'>
                ...
                <div className='d-flex justify-content-center mt-3'>
                  <button
                    ...
                    onClick={() => setStateData({modal: false, recordId: '', oldObj: {}, status: '', num: '', strDt: '', expDt: '', receiptNum: '', position: '', uscisNum: '', sevisNum: '', errMsg: ''})}
                  >
                    Back
                  </button>
                  <button onClick={handleWorkAthUpdate} disabled={disabled} type='button' className='btn btn-success'>
                    Submit
                  </button>
                </div>
              </div>
...
export default ExprtnsWorkAuPopupComponent;
