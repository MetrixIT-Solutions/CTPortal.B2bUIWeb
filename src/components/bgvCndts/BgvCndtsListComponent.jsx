...
// import { getPriority, setPriority } from '../../hooks/common';

const years = data.years;
const months = data.months;

const BgvCndtsListComponent = (props) => {
  const { rolesObj, leadsList, leadsListCount, actPgNum, searchStr, pageLimit, leadsListCountObj, status, cndtShow, loading, errMsg, disabled, empShowModal, empData, infoModal, type, usrsData, prModal, priority, statusModal, mrktngStatusModel, leadView, wStatus, notes, succMsg, email, mobCc, mobNum, visaStatus, cmnJbTitleArr, jobTitle, expYrs, expMonths, linkedIn, currentClient, previousClient, recruiter, recruitersList, cmnModal, cmnValue, cmnErrMsg, cmnType, userData, sType, filters } = props.state;
  const { handleChangeSearch, handleKeyInput, handleChangePage, handleChangeLimit, handleRouteHome, statusClick, handleRouteCons, handleCndtMenu, cndtRef, handleClickOutside, setStateData, handleClose, handlePrClick, handleStatus, handleStatusUpdate, handleCreateSkill, handleCloseFilters } = props;
  ...
  const { bgvCnt } = leadsListCountObj;
  const tCount = bgvCnt || 0;
  ...
  return (
    <div className='wrapper'>
          ...
          <div className="content-header">
            <div className="d-flex align-items-center">
              <div className="mr-auto">
                <h3 className="page-title">Background Verification</h3>
                <div className="d-inline-block align-items-center">
                  <nav>
                    <ol className="breadcrumb">
                      ...
                      <li className="breadcrumb-item" aria-current="page">Background Verification</li>
                      <li className="breadcrumb-item active" aria-current="page">List</li>
                    </ol>
                  </nav>
                </div>
              </div>
              {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[2]?.isAlwd) || rolesObj?.appAcc) &&
          ...
          <section className="content">
            <div className='row'>
              <div className='col-md-12'>
                <div className='box'>
                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[4]?.isAlwd) || rolesObj?.appAcc) && <div className='box-body'>
                    <ul className='nav nav-pills nav-fill'>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Background Verification')} className={`${status.includes('Background Verification') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.bgvCnt}</h5>
                            <p className={`${status.includes('Background Verification') ? 'mb-0' : 'mb-0'}`}>Background Verification</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                                    ...
                                    <div className='media ng-star-inserted'>
                                      <div className='media-body'>
                                            ...
                                            <div>
                                              <h5 className='padb5'>
                                                <span className='ng-star-inserted'>
                                                  <a onClick={() => window.open(`/#/consultants/background/verification/view/${item.euUser}`)}>{item.euName}</a>
                                        ...
                                        {/* <div className='jobdetails-center'>
                                          {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[10]?.isAlwd) || rolesObj?.appAcc) && <>
                                        ...
                                        <div className='jobdetails-right'>
                                          <div className='rightside-btns'>
                                            <div className='dropdown' ref={(el) => cndtRef[item._id] = el}>
                                              {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                              && <button className='btn btn-sm btn-primary me-1' onClick={() =>  handleStatus(item, 'In Job')} data-bs-toggle='tooltip'
                                              title='In Job'><i className='fa-solid fa-person-chalkboard'></i></button>}
                                              {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                              && <button className='btn btn-sm btn-primary me-1' onClick={() => handleStatus(item, 'In Marketing')} data-bs-toggle='tooltip'
                                              title='In Marketing'><i className='fa-solid fa-person-chalkboard'></i></button>}
                                              {/* {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                              && <button className='btn btn-sm btn-danger me-1' onClick={() => handleStatus(item, 'Terminated')} data-bs-toggle='tooltip'
                                              title='Terminated'><i className='fa-solid fa-user-xmark'></i></button>} */}
                                              {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[13]?.isAlwd) || rolesObj?.appAcc)
                                              ...
                                              {((rolesObj?.access?.length >= 12 && (rolesObj?.access[11]?.actions[1]?.isAlwd)) || rolesObj?.appAcc) &&
                                              <>
                                                ...
                                                {(item?.temp?.length > 0 ? (((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[16]?.isAlwd) || rolesObj?.appAcc) &&
                                                <a className='dropdown-item' onClick={() => hashHistory.push(`/checklist/${item._id}/Consultant BGV`)} data-bs-toggle='tooltip'
                                                  title='Trainy'><i className="fa-solid fa-list-check"></i>Checklist</a>)
                                                : (((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[15]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => handleStatus(item, 'Template')} data-bs-toggle='tooltip'
                                                  title='Trainy'><i className='fa-solid fa-person-chalkboard'></i>Apply Template</a>))}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                                  && <a className='dropdown-item' onClick={() => handleStatus(item, 'In Job')} data-bs-toggle="tooltip"
                                                  title='In Job'><i className="fa-solid fa-person-chalkboard"></i>In Job</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[1]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => hashHistory.push(`/consultants/background/verification/view/${item.euUser}`)} data-bs-toggle="tooltip" title="View Candidate Details"><i className="fa-regular fa-eye"></i>View Details</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                                  && <a className='dropdown-item' onClick={() => handleStatus(item, 'In Marketing')} data-bs-toggle="tooltip"
                                                  title='In Marketing'><i className="fa-solid fa-person-chalkboard"></i>In Marketing</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                                  && <a className='dropdown-item' onClick={() => handleStatus(item, 'Teminated')} data-bs-toggle="tooltip"
                                                  title='Teminated'><i className="fa-solid fa-user-xmark"></i>Teminated</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[5]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => setStateData({ leadLfcModal: true, leadView: item })} data-bs-toggle="tooltip"
                                                    title="Lead Lifecycle"><i className='fa-solid fa-rotate'></i>Lifecycle</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[13]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' data-bs-toggle="tooltip" onClick={() => setStateData({showNotes: true, leadView: item})} title="Notes"><i className='fa-solid fa-notes-medical'></i>Notes</a>}
                                                  {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[11]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' data-bs-toggle="tooltip" onClick={() => {}} title="Placed Meeting"><i className='fa-regular fa-handshake'></i>Meetings</a>}
                                                </div>
                                              </>}
                                            </div>
                                            <div className='badge badge-dark'><i className='fas fa-tasks'></i> {item.wStatus}</div>
        ...
        <Modal.Body>
          {type == 'empInfo' &&
            <div className='jobdetails-left'>
              ...
              <h6 className='ng-star-inserted'>
                <span className='ng-star-inserted'>
                  <i className='fa-brands fa-linkedin'></i> {empData?.linkedin?.length ? <a href={empData.linkedin[0].startsWith("https://") || empData.linkedin[0].startsWith("http://") ? empData.linkedin[0] : `https://${empData.linkedin[0]}`} target='_blank'>{empData.linkedin[0]}</a> : 'N/A'}
                </span>
              </h6>
            </div>}
      ...
      <Modal show={statusModal} onHide={handleClose} size='lg'>
        <Modal.Header closeButton>
          <h5 className='mb-0'>Background Verification Status Update</h5>
        </Modal.Header>
        <Modal.Body>
          ...
          <div className='row'>
            ...
            {((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[9]?.isAlwd) || rolesObj?.appAcc) && <>
              ...
              <div className='row'>
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <input type='text' disabled={true} className='form-control' placeholder='Email' value={email} onChange={(e) => setStateData({ email: e.target.value.toLowerCase().trim(), errMsg: '' })} />
                  </div>
                </div>
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <div className='d-flex'>
                      <select className='form-select form-control' disabled={true} name='mobCc' value={mobCc} onChange={(e) => setStateData({ mobCc: e.target.value })} style={{ borderBottomRightRadius: 0, borderTopRightRadius: 0, borderRight: 'none', width: 90 }}>
                        <option value='+1'>+1</option>
                        <option value='+91'>+91</option>
                      </select>
                      <input type='text' disabled={true} className='form-control' placeholder='Mob Number' name='mobNum' maxLength={10} onKeyPress={numebersOnly} value={mobNum} onChange={(e) => setStateData({ mobNum: e.target.value, errMsg: '' })} style={{ borderBottomLeftRadius: 0, borderTopLeftRadius: 0 }} />
                    </div>
                  </div>
                </div>
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <select className='form-select'  disabled={true} id='participants6' value={visaStatus} onChange={(e) => setStateData({ visaStatus: e.target.value, errMsg: '' })}>
                  ...
                  <div className='form-group'>
                    ...
                    <div className='d-flex'>
                      <Select
                        ...
                        className='w-100'
                      />
                      <button onClick={() => setStateData({ cmnModal: true, cmnType: 'Job Title' })}  disabled={true} className='btn btn-success btn-plus ml-2'><i className='fa fa-plus'></i></button>
                    </div>
                  </div>
                </div>
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <div className='row'>
                      <div className='col-md-6'>
                        {/* <label>Years</label> */}
                        <Select options={years} value={expYrs} isDisabled={true}
                          onChange={(data) => setStateData({ expYrs: data, errMsg: '' })} />
                      </div>
                      <div className='col-md-6'>
                        {/* <label>Months</label> */}
                        <Select defaultValue={months && months[1]} options={months} value={expMonths} isDisabled={true}
                ...
                <div className='col-md-6'>
                  <div className='form-group'>
                    ...
                    <input type='url' disabled={true} className='form-control' value={linkedIn} placeholder='https://www.linkedin.com/' onChange={(e) => setStateData({ linkedIn: e.target.value, errMsg: '' })} />
                  </div>
                </div>
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Current Client:</label>
                    <input type='text' disabled={true} className='form-control' value={currentClient} placeholder='Current Client' onChange={(e) => setStateData({ currentClient: firstCharCaps(e.target.value), errMsg: '' })} />
                  </div>
                </div>
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Previous Client:</label>
                    <input type='text' disabled={true} className='form-control' value={previousClient} placeholder='Previous Client' onChange={(e) => setStateData({ previousClient: firstCharCaps(e.target.value), errMsg: '' })} />
                  </div>
                </div>
              </div></>}
            <div className='text-center text-danger'>{errMsg}</div>
...
export default BgvCndtsListComponent;
