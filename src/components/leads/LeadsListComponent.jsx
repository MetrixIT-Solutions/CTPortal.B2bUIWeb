...
const LeadsListComponent = (props) => {
  const { rolesObj, userData, leadsList, leadsListCount, actPgNum, searchStr, pageLimit, leadsListCountObj, status, leadView, statusModal, wStatus, errMsg, succMsg, cndtShow, notes, recruitersList, mentorsList, mentor, recruiter, loading, template, templateList, email, visaStatus, mobNum, jobTitle, currentClient, previousClient, mobCc, linkedIn, expYrs, expMonths, empData, mrktngStatusModel, disabled, infoModal, type, cmnModal, cmnValue, cmnErrMsg, cmnType, cmnJbTitleArr, prModal, priority, usrsData, reAssModal, mentrsData, sType, filters } = props.state;
  const { handleChangeSearch, handleKeyInput, handleChangePage, handleChangeLimit, handleRouteHome, statusClick, handleRouteCons, handleClose, handleStatusUpdate, setStateData, handleCndtMenu, cndtRef, handleClickOutside, handleStatus, handleTemplateUpdate, handleMarketingInfo, handleInvMeetings, handleCreateSkill, handlePrClick, handleReassignMentor, handleMentorChange, handleUpdateMentor, handleCloseFilters} = props;
  ...
  const { approvedCnt, trainyCnt, jobEndTrainyCnt, screenyCnt, sendToMrktngCnt, jobEndedSndToMrktng } = leadsListCountObj;
  const tCount = approvedCnt + trainyCnt + jobEndTrainyCnt + screenyCnt + sendToMrktngCnt + jobEndedSndToMrktng;
  ...
  return (
    <div className='wrapper'>
          ...
          <div className='content-header'>
            <div className='d-flex align-items-center'>
              <div className='mr-auto'>
                <h3 className='page-title'>On Boarding</h3>
                <div className='d-inline-block align-items-center'>
                  <nav>
                    <ol className='breadcrumb'>
                      ...
                      <li className='breadcrumb-item' aria-current='page'>On Boarding</li>
                      <li className='breadcrumb-item active' aria-current='page'>List</li>
                    </ol>
                  </nav>
                </div>
              </div>
              {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[2]?.isAlwd) || rolesObj?.appAcc) &&
          ...
          <section className='content'>
            <div className='row'>
              <div className='col-md-12'>
                <div className='box'>
                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[4]?.isAlwd) || rolesObj?.appAcc) && <div className='box-body'>
                      ...
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Approved')} className={`${status.includes('Approved') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.approvedCnt}</h5>
                            <p className={`${status.includes('Approved') ? 'mb-0' : 'mb-0'}`}>Approved</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Trainy')} className={`${status.includes('Trainy') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-50'>{leadsListCountObj.trainyCnt}</h5>
                            <p className={`${status.includes('Trainy') ? 'mb-0' : 'mb-0'}`}>Trainy </p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Job Ended Trainy')} className={`${status.includes('Job Ended Trainy') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.jobEndTrainyCnt}</h5>
                            <p className={`${status.includes('Job Ended Trainy') ? 'mb-0' : 'mb-0'}`}>Job Ended Trainy </p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Send To Marketing')} className={`${status.includes('Send To Marketing') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.sendToMrktngCnt}</h5>
                            <p className={`${status.includes('Send To Marketing') ? 'mb-0' : 'mb-0'}`}>Send To Marketing</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Job Ended Send To Marketing')} className={`${status.includes('Job Ended Send To Marketing') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.jobEndedSndToMrktng}</h5>
                            <p className={`${status.includes('Job Ended Send To Marketing') ? 'mb-0' : 'mb-0'}`}>Job Ended Send To Marketing</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                                      ...
                                      <div className='jobdetails-center'>
                                        {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[14]?.isAlwd) || rolesObj?.appAcc) && <>
                                          <h5 className='ng-star-inserted'>
                                            ...
                                            <label className='ng-star-inserted'>
                                              <CommonTooltip type='Icon' name='' className='fa-solid fa-briefcase' title='Experience' display='top' /> {(mYears || mMonths) ? mYears + '.' + mMonths + ' Years' : 'Fresher'}
                                            </label>
                                          </h5>
                                          <h6 className='ng-star-inserted'>
                                        ...
                                        <h6 className='ng-star-inserted'>
                                          ...
                                          <span className='ng-star-inserted'>
                                            ...
                                            {/* <span><a onClick={() => setStateData({ infoModal: true, type: 'mentor', empData: {title: item.euName + ' ' + '(' + refUID + ')', name: item.onlName, email: item.onlEmID, mobile: item.onlMobCcNum }})}>{item.onlName ? item.onlName : ''} </a></span>
                                            <span><a onClick={() => setStateData({ infoModal: true, type: 'mentor', empData: {title: item.euName + ' ' + '(' + refUID + ')', name: item.onmName, email: item.onmEmID, mobile: item.onmMobCcNum }})}>{item.onmName ? item.onmName : ''} </a></span> */}
                                          </span>
                                        </h6>
                                      </div>
                                      <div className='jobdetails-right'>
                                        <div className='rightside-btns'>
                                          <div className='dropdown' ref={(el) => cndtRef[item._id] = el}>
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Approved' || item.wStatus == 'Send To Marketing') && <button className='btn btn-sm btn-warning me-1' onClick={() => handleStatus(item, 'Trainy')} data-bs-toggle='tooltip'
                                                title='Trainy'><i className='fa-solid fa-person-chalkboard'></i></button>}
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Approved' || item.wStatus == 'Trainy') && <button className='btn btn-sm btn-primary me-1' onClick={() => handleStatus(item, 'Send To Marketing')} data-bs-toggle='tooltip'
                                                title='Send To Marketing'><i className='fa-solid fa-bullhorn'></i></button>}

                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Job Ended' || item.wStatus == 'Job Ended Send To Marketing') && <button className='btn btn-sm btn-warning me-1' onClick={() => handleStatus(item, 'Job Ended Trainy')} data-bs-toggle='tooltip'
                                                title='Job Ended Trainy'><i className='fa-solid fa-person-chalkboard'></i></button>}
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Job Ended' || item.wStatus == 'Job Ended Trainy') && <button className='btn btn-sm btn-primary me-1' onClick={() => handleStatus(item, 'Job Ended Send To Marketing')} data-bs-toggle='tooltip'
                                                title='Job Ended Send To Marketing'><i className='fa-solid fa-bullhorn'></i></button>}
                                            {/* {((rolesObj?.access?.length >= 10 && rolesObj?.access[9]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                                && <button className='btn btn-sm btn-danger me-2' onClick={() => handleStatus(item, 'Terminated')} data-bs-toggle="tooltip"
                                                  title='Terminate'><i className="fa-solid fa-user-xmark"></i></button>} */}
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Send To Marketing' || item.wStatus == 'Job Ended Send To Marketing') && !item.ofsVrfy && <button className='btn btn-sm btn-warning me-1' onClick={() => hashHistory.push(`/consultants/onboarding/view/${item.euUser}`)} data-bs-toggle='tooltip'
                                                title='Offshore Review'><i className='fas fa-users-cog'></i></button>}
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus == 'Send To Marketing' || item.wStatus == 'Job Ended Send To Marketing') && item.ofsVrfy && <button className='btn btn-sm btn-warning me-1' onClick={() => hashHistory.push(`/consultants/onboarding/view/${item.euUser}`)} data-bs-toggle='tooltip'
                                                title='Onsite Review'><i className='fas fa-users-cog'></i></button>}

                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[13]?.isAlwd) || rolesObj?.appAcc)
                                              && (item.wStatus === 'Trainy' || item.wStatus == 'Job Ended Trainy') && <button className='btn btn-sm btn-warning me-1' onClick={() => handleMarketingInfo(item)} data-bs-toggle='tooltip'
                                                title='Set Marketing Info'><i className="fa-solid fa-file-audio"></i></button>}
                                            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[11]?.isAlwd) || rolesObj?.appAcc)
                                            ...
                                            {/* {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[13]?.isAlwd) || rolesObj?.appAcc) */}
                                            {/* {((rolesObj?.access?.length >= 8 && rolesObj?.access[7]?.actions[5]?.isAlwd) || rolesObj?.appAcc) &&  <button className='btn btn-sm btn-warning me-1' onClick={() => handleReassignMentor(item)} data-bs-toggle='tooltip'
                                                title='Reassign Mentor'><i className="fa-solid fa-users-rectangle"></i></button> } */}
                                            {/* ================================================================Menu================================================================ */}
                                            {((rolesObj?.access?.length >= 9 && (rolesObj?.access[8]?.actions[1]?.isAlwd || rolesObj?.access[8]?.actions[4]?.isAlwd || rolesObj?.access[8]?.actions[5]?.isAlwd || rolesObj?.access[8]?.actions[6]?.isAlwd || rolesObj?.access[8]?.actions[7]?.isAlwd || rolesObj?.access[8]?.actions[8]?.isAlwd || rolesObj?.access[8]?.actions[9]?.isAlwd || rolesObj?.access[8]?.actions[11]?.isAlwd || rolesObj?.access[8]?.actions[13]?.isAlwd)) || rolesObj?.appAcc) &&
                                              <>
                                                ...
                                                {(item?.temp?.length > 0 ? (((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[16]?.isAlwd) || rolesObj?.appAcc) &&
                                                  <a className='dropdown-item' onClick={() => hashHistory.push(`/checklist/${item._id}/Consultant On Boarding`)} data-bs-toggle='tooltip'
                                                    title='Trainy'><i className="fa-solid fa-list-check"></i>Checklist</a>)
                                                  : (((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[15]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => handleStatus(item, 'Template')} data-bs-toggle='tooltip'
                                                    title='Trainy'><i className='fa-solid fa-person-chalkboard'></i>Apply Template</a>))}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Approved' || item.wStatus == 'Send To Marketing' || item.wStatus == 'In Marketing') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Trainy')} data-bs-toggle='tooltip'
                                                      title='Trainy'><i className='fa-solid fa-person-chalkboard'></i>Trainy</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Approved' || item.wStatus == 'Trainy') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Send To Marketing')} data-bs-toggle='tooltip'
                                                      title='Send To Marketing'><i className='fa-solid fa-bullhorn'></i>Send To Marketing</a>}

                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Trainy') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Approved')} data-bs-toggle='tooltip'
                                                      title='Approved'><i className='fa-solid fa-user-check'></i>Approved</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Job Ended' || item.wStatus == 'Job Ended Send To Marketing' || item.wStatus == 'Job Ended Marketing') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Job Ended Trainy')} data-bs-toggle='tooltip'
                                                      title='Job Ended Trainy'><i className='fa-solid fa-arrows-rotate'></i>Job Ended Trainy</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[6]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Job Ended Trainy' || item.wStatus == 'Job Ended') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Job Ended Send To Marketing')} data-bs-toggle='tooltip'
                                                      title='Job Ended Send To Marketing'><i className='fa-solid fa-bullhorn'></i>Job Ended Send To Marketing</a>}

                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[13]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Approved' || item.wStatus === 'Trainy' || item.wStatus == 'Job Ended' || item.wStatus == 'Job Ended Trainy') && <a className='dropdown-item' onClick={() => handleMarketingInfo(item)} data-bs-toggle='tooltip'
                                                      title='Set Marketing Info'><i className="fa-solid fa-file-audio"></i>Set Marketing Info</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[5]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Job Ended Trainy') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Job Ended')} data-bs-toggle='tooltip'
                                                      title='Job Ended'><i className='fa-solid fa-person-circle-xmark'></i>Job Ended</a>}

                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Send To Marketing' || item.wStatus == 'Job Ended Send To Marketing') && !item.ofsVrfy && <a className='dropdown-item' onClick={() => hashHistory.push(`/consultants/onboarding/view/${item.euUser}`)} data-bs-toggle='tooltip'
                                                      title='Offshore Review'><i className='fas fa-users-cog'></i>Offshore Review</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Send To Marketing' || item.wStatus == 'Job Ended Send To Marketing') && item.ofsVrfy && <a className='dropdown-item' onClick={() => hashHistory.push(`/consultants/onboarding/view/${item.euUser}`)} data-bs-toggle='tooltip'
                                                      title='Onsite Review'><i className='fas fa-users-cog'></i>Onsite Review</a>}
                                                  ...
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[1]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => hashHistory.push(`/consultants/onboarding/view/${item.euUser}`)} data-bs-toggle='tooltip'
                                                    title='View Candidate Details'><i className='fa-regular fa-eye'></i>View Details</a>}
                                                  {((rolesObj?.access?.length >= 10 && rolesObj?.access[9]?.actions[7]?.isAlwd) || rolesObj?.appAcc)
                                                    && <a className='dropdown-item' onClick={() => handleStatus(item, 'Terminated')} data-bs-toggle="tooltip"
                                                    title='Terminate'><i className="fa-solid fa-user-xmark"></i>Terminate</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[4]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => setStateData({ leadLfcModal: true, leadView: item })} data-bs-toggle="tooltip"
                                                    title="Lifecycle"><i className='fa-solid fa-rotate'></i>Lifecycle</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[9]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => handleInvMeetings(item)} data-bs-toggle="tooltip"
                                                    title="On Boarding Meeting"><i className="fa-regular fa-handshake"></i>Meeting</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[11]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => setStateData({ showNotes: true, leadView: item })} data-bs-toggle="tooltip"
                                                    title="On Boarding Notes"><i className='fa-solid fa-notes-medical'></i>Notes</a>}

                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'In Marketing') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Terminate')} data-bs-toggle='tooltip'
                                                      title='Terminate'><i className='fa-solid fa-user-xmark'></i>Terminate</a>}
                                                  {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[8]?.isAlwd) || rolesObj?.appAcc)
                                                    && (item.wStatus == 'Job Ended' || item.wStatus == 'Job Ended Marketing') && <a className='dropdown-item' onClick={() => handleStatus(item, 'Job Ended Terminate')} data-bs-toggle='tooltip'
                                                      title='Job Ended Terminated'><i className='fa-solid fa-user-xmark'></i>Job Ended Terminate</a>}
                                                </div>
                                              </>}
                                          </div>
                                        </div>
      ...
      <Modal show={statusModal} onHide={handleClose} size='lg'>
        <Modal.Header closeButton>
          <h5 className='mb-0'>{mrktngStatusModel ? 'Set Marketing Info' : 'On Boarding Status Update'} </h5>
        </Modal.Header>
        <Modal.Body>
          ...
          {/* <form> */}
          <div className='row'>
            {(!mrktngStatusModel && wStatus !== 'Template') && <div className='col-md-5'>
            ...
            {!mrktngStatusModel && <div className='col-md-7'>
              <div className='form-group'>
            ...
            {!mrktngStatusModel && wStatus == 'In Marketing' &&
            ...
            {!mrktngStatusModel && leadView.wStatus == 'Job Ended' && (wStatus == 'Job Ended Trainy' || wStatus == 'Send To Marketing') &&
              <div className='col-md-6'>
                <div className='form-group'>
            ...
            {((rolesObj?.access?.length >= 9 && rolesObj?.access[8]?.actions[13]?.isAlwd) || rolesObj?.appAcc) && <>
              ...
              <div className='row'>
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Email:</label>{(wStatus == 'Send To Marketing' || mrktngStatusModel) && <span className='text-danger'>*</span>}
                ...
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Mob Number:</label>{(wStatus == 'Send To Marketing' || mrktngStatusModel) && <span className='text-danger'>*</span>}
                    <div className='d-flex'>
                ...
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Visa Status:</label>{(wStatus == 'Send To Marketing' || mrktngStatusModel) && <span className='text-danger'>*</span>}
                ...
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>Job Title:</label>{(wStatus == 'Send To Marketing' || mrktngStatusModel) && <span className='text-danger'>*</span>}
                    <div className='d-flex'>
                      <Select
                        isClearable
                ...
                <div className='col-md-6'>
                  <div className='form-group'>
                    <label className='form-lable'>LinkedIn URL:</label>{(wStatus == 'Send To Marketing' || mrktngStatusModel) && <span className='text-danger'>*</span>}
              ...
              <div className='col-md-6'>
                <div className='form-group'>
                  ...
                  <select className='form-select' value={template} onChange={(e) => setStateData({ template: e.target.value, errMsg: '' })}>
                    <option value=''>Select</option>
                    {templateList && templateList.length > 0 && templateList.map((item, i) => <option key={i} value={item._id}>{item.tempName}</option>)}
                  </select>
                </div>
              </div>}
          ...
          <div className='text-end'>
            ...
            {wStatus !== 'Template' && <button type='button' className='btn btn-success' disabled={disabled} onClick={handleStatusUpdate}>Update</button>}
            {wStatus == 'Template' && <button type='button' className='btn btn-success' onClick={handleTemplateUpdate}>Apply</button>}
          </div>
          {/* </form> */}
        </Modal.Body>
      </Modal>
      ...
      <Modal show={reAssModal} onHide={() => setStateData({ reAssModal: false, errMsg: '', mentor: '', mentorsList: [], notes: '' })} size='md' centered>
        <Modal.Header closeButton={() => setStateData({ reAssModal: false, errMsg: '', mentor: '', mentorsList: [], notes: '' })}>
        ...
        <Modal.Footer>
          <div className='text-end'>
            <Button type='button' className='btn btn-danger me-2' disabled={disabled} onClick={() => setStateData({ reAssModal: false, errMsg: '', mentor: '', mentorsList: [], disabled: false, notes: '' })}>Close</Button>
