...
const H1BPetitionsCreateComponent = (props) => {
  const {pType, benType, fName, lName, emID, mobCC, mobNum, referredBy, otherRefer, refUser, usrsList, cnsltantsList, isdisable, cnsltUsr, errMsg, rprtId, orgsList, userInfo, orgId, teams, teamId, template, tempList} = props.state;
  const { setStateData, handleClose, createModal, H1BPetitionsCreate, handleChangeReporter, handleChangeOrg, handleChangeTeams, handleInputChange } = props;
  return (
    <div>
      {/* create modal */}
      <Modal show={createModal} onHide={handleClose} size='lg' aria-labelledby='example-modal-sizes-title-lg' >
        <Modal.Header closeButton>
          <Modal.Title>H1B Petition Create</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className='row' >
            ...
            <div className='col-md-12'>
              <div className="form-group">
                <label>Petition Type</label><span className='text-danger'>*</span>
                <select className="form-select form-control" name='pType' value={pType} onChange={(e) => setStateData({ pType: e.target.value, errMsg: '' })}>
                  <option value=''>Select</option>
                  <option value='Cap Petition'>Cap Petition</option>
                  <option value='Extension'>Extension</option>
                  <option value='Extension + Amendment'>Extension + Amendment</option>
          ...
          <div className='row'>
            ...
            <div className='row'>
              ...
              {referredBy == 'Internal' && <div className='col-md-6'>
                <div className='form-group'>
                  <select className='form-select form-control' value={refUser} onChange={(e) => setStateData({ refUser: e.target.value, errMsg: '' })}>
                    <option value=''>Select</option>
          ...
          <div className='d-flex justify-content-center'>
            ...
            <button className='btn btn-primary text-center' disabled={isdisable} onClick={(e) => H1BPetitionsCreate(e)}>Create</button>
          </div>
        </Modal.Body>
      </Modal>
    </div>
...
export default H1BPetitionsCreateComponent