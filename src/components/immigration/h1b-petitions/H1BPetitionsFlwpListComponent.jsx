...
const H1BPetitionsFlwpListComponent = (props) => {
  const {Notes, fwNotes, FwlpsModal, errMsg, disable, ptnsFlwpsList, FwlerrMsg, fwlpsNotesModal, ptnsData, refId, rolesObj} = props.state;
  const {setStateData, handleClose, ptnsFollowupsCreate, toggleModal, handleFollowupsEdit, handleFwlpsNtsUpdate} = props;
    ...
    <div>
      ...
      <Offcanvas show={FwlpsModal} onHide={handleClose} backdrop='static' className='w-50 submit-modal'>
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>H1B Petition Notes</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          {/* <p className='mb-0'><b>Candidate Info:</b> {ptnsData.euName} | {ptnsData.euEmID} | {ptnsData.euMobCcNum}</p> <hr /> */}
          ...
          {((rolesObj?.access?.length >= 20 && rolesObj?.access[19]?.actions[8]?.isAlwd) || rolesObj?.appAcc) &&
            <>
              ...
              <form>
                ...
                <div>
                  ...
                  <Button variant='success' disabled={disable} onClick={ptnsFollowupsCreate}>
                    Create
                  </Button>
                </div>
              </form>
          ...
          <strong>H1B Petition Notes List</strong> <br />
          <div className='container mt-4'>
            <div className='row'>
              {ptnsFlwpsList && ptnsFlwpsList.length > 0 ? (
                ptnsFlwpsList.map((item, index) => {
                    ...
                    <div className='col-md-6' key={index}>
                      <div className='card mb-6'>
                        <div className=' d-flex justify-content-between m-2'>
                          ...
                          {((rolesObj?.access?.length >= 20 && rolesObj?.access[19]?.actions[12]?.isAlwd) || rolesObj?.appAcc) && sed >= ptm && (
                            <div>
...
export default H1BPetitionsFlwpListComponent;
