...
const TemplateCreateComponent = (props) => {
  const { orgsList, teamsList, rolesList, userInfo, usersList, orgId, teamId, roleId, userId, disable, errMsg, rolesObj, tStatus, tempName, tempSeq, tempCat, tempNotes, tempData, errIndex, tempErrMsg, showCases, sectionData } = props.state;
  const { setStateData, handleChangeOrg, handleChangeTeam, handleChangeRole, tempOnChange, handleAddField, handleDeleteField, handleCreate, isManClick, tempCatChange, handleSelection, tableInptChange, handleAddCase, addRow, deleteRow, handleCloseCase, addSubHeading, dltSubHeading, tableChange, isTableManClick } = props;  
  return (
    <div className='wrapper'>
                    ...
                    <div className='bs-stepper-content'>
                      <div className='row'>
                        {rolesObj?.appAcc &&
                            ...
                            <div className='col-md-3'>
                              <div className='form-group'>
                                ...
                                <Select isMulti options={rolesList} value={roleId} onChange={handleChangeRole} />                              </div>
                            </div>
                            <div className='col-md-3'>
                              <div className='form-group'>
                                <label>Teams</label>
                                <select className='form-select form-control' value={teamId} onChange={handleChangeTeam}>
                                  <option value=''>Select</option>
                                  {teamsList && teamsList.length > 0 && teamsList.map((item, i) => <option key={i} value={item._id}>{item.oTeam}</option>)}
                                </select>
                              </div>
                            </div>
                          </>}
                        <div className='col-md-3'>
                          <div className='form-group'>
                            <label>Status</label>
                            <select className='form-select form-control' value={tStatus} onChange={(e) => setStateData({ tStatus: e.target.value, errMsg: '' })}>
                              <option value=''>Select</option>
                        ...
                        <div className='col-md-3'>
                          <div className='form-group'>
                            <label>Category</label><span className='text-danger'>*</span>
                              <Select isMulti options={dTypes.tempCategory} value={tempCat} onChange={tempCatChange} />
                          </div>
                        </div>
                        <div className='col-md-3'>
                          <div className='form-group'>
                            <label>Template Name</label><span className='text-danger'>*</span>
                            <input type='text' className='form-control' placeholder='Template Name' value={tempName} onChange={(e) => setStateData({ tempName: initCaps(e.target.value), errMsg: '' })} />
                          </div>
                        </div>
                        <div className='col-md-3'>
                          <div className='form-group'>
                            <label>Sequence</label><span className='text-danger'>*</span>
                            <input type='text' className='form-control' placeholder='Sequence' maxLength={3} value={tempSeq} onKeyPress={numebersOnly} onChange={(e) => setStateData({ tempSeq: e.target.value, errMsg: '' })} />
                          </div>
                        </div>
                        <div className='col-md-6'>
                          <div className='form-group'>
                            <label>Notes</label>
                            <textarea name='decisions' id='decisions3' rows='4' className='form-control' value={tempNotes} onChange={(e) => setStateData({ tempNotes: firstCharCaps(e.target.value), errMsg: '' })}></textarea>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='d-flex justify-content-center mt-2'>
                        <button className='btn btn-danger mr-2' onClick={() => hashHistory.push('/templates')}>Back</button>
      ...
      <Modal show={showCases} onHide={handleCloseCase} size='lg'>
        ...
        <Modal.Body>
          {/* {tempData?.length > 0 && tempData.map((item, mainIndex, mainArr) => {
            const tableArr = item?.dataObj?.tableArr
