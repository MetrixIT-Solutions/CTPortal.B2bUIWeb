...
const TerminatedCndtsListComponent = (props) => {
  const { rolesObj, leadsList, leadsListCount, actPgNum, searchStr, pageLimit, leadsListCountObj, status, cndtShow, loading, errMsg, disabled, empShowModal, empData, infoModal, type, usrsData, prModal, priority, userData, sType, filters } = props.state;
  const { handleChangeSearch, handleKeyInput, handleChangePage, handleChangeLimit, handleRouteHome, statusClick, handleRouteCons, handleCndtMenu, cndtRef, handleClickOutside, setStateData, handleClose, handlePrClick, handleTrmntdInMarketing, handleCloseFilters } = props;
  ...
  const { terminatedCnt, jobEndedTerminatedCnt } = leadsListCountObj;
  const tCount = terminatedCnt + jobEndedTerminatedCnt;
          ...
          <div className="content-header">
            <div className="d-flex align-items-center">
              <div className="mr-auto">
                <h3 className="page-title">Terminated</h3>
                <div className="d-inline-block align-items-center">
                  <nav>
                    <ol className="breadcrumb">
                      ...
                      <li className="breadcrumb-item" aria-current="page">Terminated</li>
                      <li className="breadcrumb-item active" aria-current="page">List</li>
                    </ol>
                  </nav>
                </div>
              </div>
              {((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[2]?.isAlwd) || rolesObj?.appAcc) &&
          ...
          <section className="content">
            <div className='row'>
              <div className='col-md-12'>
                <div className='box'>
                  {((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[4]?.isAlwd) || rolesObj?.appAcc) && <div className='box-body'>
                    <ul className='nav nav-pills nav-fill'>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Terminated')} className={`${status.includes('Terminated') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.terminatedCnt}</h5>
                            <p className={`${status.includes('Terminated') ? 'mb-0' : 'mb-0'}`}>Terminated</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                        <a onClick={() => statusClick('Job Ended Terminated')} className={`${status.includes('Job Ended Terminated') ? 'nav-link active' : 'nav-link'}`}>
                          <div className='text-center'>
                            <h5 className='font-weight-500'>{leadsListCountObj.jobEndedTerminatedCnt}</h5>
                            <p className={`${status.includes('Job Ended Terminated') ? 'mb-0' : 'mb-0'}`}>Job Ended Terminated</p>
                          </div>
                        </a>
                      </li>
                      <li className='nav-item b-1'>
                                      ...
                                      <div className='media-body'>
                                        ...
                                        <div className='jobdetails-right'>
                                          <div className='rightside-btns'>
                                            {((rolesObj?.access?.length >= 14 && (rolesObj?.access[13]?.actions[1]?.isAlwd)) || rolesObj?.appAcc) &&
                                              <div className="dropdown" ref={(el) => cndtRef[item._id] = el}>
                                                ...
                                                <div className={classnames('dropdown-menu dropdown-menu-right', { 'show': cndtShow === item._id })}>
                                                  {((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[1]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => hashHistory.push(`/consultants/terminated/view/${item.euUser}`)} data-bs-toggle="tooltip" title="View Candidate Details"><i className="fa-regular fa-eye"></i>View Details</a>}
                                                  { ((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[8]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' data-bs-toggle="tooltip" onClick={() => setStateData({showNotes: true, leadView: item})} title="Notes"><i className='fa-solid fa-notes-medical'></i>Notes</a>}
                                                  {((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[6]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' data-bs-toggle="tooltip" onClick={() => handleTrmntdInMarketing(item)} title="Terminated Meetings"><i className='fa-regular fa-handshake'></i>Meetings</a>}
                                                  {((rolesObj?.access?.length >= 14 && rolesObj?.access[13]?.actions[5]?.isAlwd) || rolesObj?.appAcc) && <a className='dropdown-item' onClick={() => setStateData({ leadLfcModal: true, leadView: item })} data-bs-toggle="tooltip"
...
export default TerminatedCndtsListComponent;
