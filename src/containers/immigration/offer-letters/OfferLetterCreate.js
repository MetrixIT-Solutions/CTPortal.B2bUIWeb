...
import OfferLetterCreateComponent from '../../../components/immigration/offer-letters/OfferLetterCreateComponent';
...
import { postB2BOfrLtrCreate } from '../../../actions/immigration/OfferLetterActions';
...
class OfferLetterCreate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      orgsList: [],
  ...
  componentDidMount = async () => {
    ...
    if(userInfo?.userType === 'Employee') {
      ...
      const reqBody = { userRole: userInfo.userRole, user: '', team: teamId, org: '', tempCat: 'Immigration Offer Letter' };
  ...
  handleChangeOrg = (e) => {
    ...
    this.setState({ usrsList: [], orgId, errMsg: '', rprtId: '', teams: [], teamId: '', cnsltantsList: [], cnsltUsr: '', template:'', tempList: [] });
  }
  handleChangeReporter = (e) => {
    ...
    if(teamId) {
      ...
      this.getCnsltantsList(rprtId, teamIds, '');
      if (userInfo.userType === 'Employee') {
        const reqBody = { userRole: userInfo.userRole, user: '', team: teamId, teamIds, org: orgId, tempCat: 'Immigration Offer Letter' };
        teamId && this.tempApiCall(reqBody);
      } else {
        const reqBody = { userRole: user.userRole, user: rprtId, team: teamId, teamIds, org: orgId, tempCat: 'Immigration Offer Letter' };
        teamId && this.tempApiCall(reqBody);
      }
    } else this.setState({ rprtId, errMsg: '', teams: user?.teams || [], teamId, teamIds: [], cnsltantsList: [], cnsltUsr: '', template:'', tempList:[] });
  }
  handleChangeTeams = (e) => {
    const { usrsList, rprtId, userInfo, orgId, orgTeamsList  } = this.state;
    ...
    if (userInfo.userType === 'Employee') {
      ...
      const reqBody = { userRole: userObj.userRole, user: rprtId, team: teamId, teamIds, org: orgId, tempCat: 'Immigration Offer Letter' };
      teamId && this.tempApiCall(reqBody)
    }
  }

  offerLetterCreate = (event) => {
    const { userInfo, benType, fName, lName, emID, mobNum, refUser, cnsltUsr, referredBy, otherRefer, cnsltantsList, usrsList, orgsList, rprtId, orgId, mobCC, teams, teamId, tempList, template } = this.state;
    ...
    if (userInfo?.userType === 'App' && !orgId) {
      ...
      const reqBody = {
        ...
        pDate, emID, mobNum, mobCc: mobNum ? mobCC : '',
        ...
        temp:[{
          _id: template,
          tempCat: 'Immigration Offer Letter',
      ...
      this.props.postB2BOfrLtrCreate(reqBody, (resObj) => {
        if (resObj.status == '200') {
          const tms = userInfo?.userType == 'Employee' ? (userInfo?.teams?.length ? userInfo.teams : []) : [];
          const tId = tms.length === 1 ? tms[0]._id : '';
          this.props.handleClose();
          this.props.getOfrLtrListData(1, '', 10, false);
          this.setState({teams: tms, teamId: tId, orgId: '', rprtId: '', cnsltUsr: '', cnsltantsList: tId ? cnsltantsList : [], benType: '', fName: '', lName: '', emID: '', mobCC: '', mobNum: '', referredBy: 'Other', otherRefer: '', refUser: ''});
        } else if (resObj.status == '103') {
          this.setState({ errMsg: resObj.resData.message, isdisable: false  });
        } else {
          this.setState({ errMsg: 'Offer Letter Create Failed', isdisable: false });
        }
      });
    }
  ...
  handleClose = () => {
    ...
    const teamId = teams.length === 1 ? teams[0]._id : '';
    this.setState({ teams, teamId, orgId: '', rprtId: '', cnsltUsr: '', cnsltantsList: teamId ? cnsltantsList : [], benType: '', fName: '', lName: '', emID: '', mobCC: '', mobNum: '', referredBy: 'Other', otherRefer: '', refUser: ''});
    this.props.handleClose();
  }
  ...
  render() {
    return (
      <OfferLetterCreateComponent
        state={this.state}
        setStateData={this.setStateData}
        createModal={this.props.createModal}
        handleClose={this.handleClose}
        offerLetterCreate={this.offerLetterCreate}
...
const mapDistachToProps = (dispatch) => ({
  ...
  postB2BOfrLtrCreate: (body, callback) => dispatch(postB2BOfrLtrCreate(body, callback)),
...
export default connect(mapStateToProps, mapDistachToProps)(OfferLetterCreate);
