...
import { BgvCndtsListComponent } from '../../components/bgvCndts';
...
class BgvCndtsList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      leadsList: [],
  ...
  getleads = async (actPgNum, searchStr, pageLimit, isSearch, status) => {
    ...
    const reqBody = {
      ...
      status,
      wStatus: ['Background Verification']
    };
    this.props.postB2BLeadsList(reqBody, (resObj) => {
      if (resObj.status == '200') {
        ...
        const leadsListCountObj = { bgvCnt: 0 };
  ...
  setStatus = (resData) => {
    let bgvCnt = 0
    const bgvObj = resData?.find(item => item._id == "Background Verification");
    bgvCnt = bgvObj ? bgvObj.count : 0;
    return { bgvCnt };
  }
  ...
  render() {
    ...
    const ncAccess = ((rolesObj?.access?.length >= 12 && rolesObj?.access[11]?.actions[9]?.isAlwd) || rolesObj?.appAcc);

    return <>
      <BgvCndtsListComponent
        state={this.state}
      ...
      <LeadsNotes showNotes={this.state.showNotes} leadData={this.state.leadView} type='Background Verification' setStateData={this.setStateData} ncAccess={ncAccess} />
      <LeadsApplyTemp appTemp={this.state.appTemp} lead={this.state.leadView} userData={this.state.userData} setStateData={this.setStateData} leadsList={this.state.leadsList} tempCat="Consultant BGV"/>
    </>
  };
};
...
export default connect(mapStateToProps, mapDistachToProps)(BgvCndtsList);