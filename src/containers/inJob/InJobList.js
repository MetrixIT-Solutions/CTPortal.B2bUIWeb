...
import { InJobListComponent } from '../../components/inJob';
...
class InJobList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      leadsList: [],
  ...
  getleads = async (actPgNum, searchStr, pageLimit, isSearch, status) => {
    ...
    this.props.postB2BLeadsList(reqBody, (resObj) => {
      if (resObj.status == '200') {
        ...
        const leadsListCountObj = { injobCnt: 0 };
  ...
  setStatus = (resData) => {
    let injobCnt = 0
    const bgvObj = resData?.find(item => item._id == "In Job");
    injobCnt = bgvObj ? bgvObj.count : 0;
    return { injobCnt }
  }
  ...
  render() {
    ...
    const ncAccess = ((rolesObj?.access?.length >= 13 && rolesObj?.access[12]?.actions[13]?.isAlwd) || rolesObj?.appAcc);
    ...
    return <>
      ...
      <LeadsNotes showNotes={this.state.showNotes} leadData={this.state.leadView} type='In Job' setStateData={this.setStateData} ncAccess={ncAccess} />
    </>
  };
};
...
export default connect(mapStateToProps, mapDistachToProps)(InJobList);